...
namespace MiniShop.UI.Areas.Admin.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> AssignRoles(UserRolesViewModel userRolesViewModel)
        {
            ...
            ModelState.AddModelError("", "En az bir kategori seçmelisiniz.");
            return View(userRolesViewModel);
        }
        ...
        {
            var roleNames = await _roleManager.Roles.Select(r=>new AssignRoleViewModel
            {
                 RoleId=r.Id,
                  RoleName = r.Name,
                   IsAssigned = false
        ...
        public async Task<IActionResult> Create(AddUserViewModel addUserViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _userManager.CreateAsync(newUser, "Qwe123.");
                //var selectedRoleList = addUserViewModel.Roles.Where(y=>y.IsAssigned).Select(x => x.RoleName).ToList();
                //await _userManager.AddToRolesAsync(newUser, selectedRoleList);
                if(result.Succeeded)
                {
            ...
            //    IsAssigned = false
            //}).ToListAsync();
            //addUserViewModel.Roles = roleNames;
