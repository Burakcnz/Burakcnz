...
namespace MiniShopUI.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(AddProductViewModel addProductViewModel, IFormFile image)
        {
            if(ModelState.IsValid && addProductViewModel.CategoryIds.Count>0 && image!=null)
            {
                ...
                using (var stream = image.OpenReadStream())
                {
                    ...
                    HttpResponseMessage httpResponseMessageImage = await httpClient.PostAsync("http://localhost:5100/Products/ImageUpload", imageContent);
                    string httpResponseMessageImageUrl = await httpResponseMessageImage.Content.ReadAsStringAsync();

                    if (httpResponseMessageImage.IsSuccessStatusCode)
                    {
                        if (httpResponseMessageImageUrl != "")
                        {
                            //Ürün Kaydetme
                            addProductViewModel.ImageUrl = httpResponseMessageImageUrl;
            ...
            ViewBag.ImageErrorMessage = image == null ? "Lütfen bir resim yükleyiniz" : null;
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(EditProductViewModel editProductViewModel, IFormFile image)
        {
            ...
            if(ModelState.IsValid && editProductViewModel.CategoryIds.Count > 0)
            {
                ...
                //Resim Yükleme
                using (var stream = image == null ? null : image.OpenReadStream())
                {
                    HttpClient httpClient = new HttpClient();
                    if (stream != null)
                    {
                        ...
                        if (httpResponseMessageImage.IsSuccessStatusCode)
                        {
                            if (httpResponseMessageImageUrl != "")
                            {
                                editProductViewModel.ImageUrl = httpResponseMessageImageUrl;
                            }
                        }
                        else
                        {
                            ViewBag.ImageErrorMessage = "Lütfen geçerli formatta bir resim yükleyiniz.(png, jpg, jpeg)";
                        }
                    }
